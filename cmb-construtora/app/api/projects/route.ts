...
export async function GET() {
  try {
    const projects = await sql`SELECT * FROM projects ORDER BY featured DESC, created_at DESC`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [project] = await sql`
      INSERT INTO projects (title, description, category, location, area_m2, year_completed, status, featured, cover_image, images)
      VALUES (
        ${title},
        ${description || null},
        ${category || null},
