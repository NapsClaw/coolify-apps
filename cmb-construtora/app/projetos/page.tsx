...
async function getProjects(category?: string) {
  try {
    if (category) {
      return await sql`SELECT * FROM cmb_projects WHERE category = ${category} ORDER BY created_at DESC`
    }
    return await sql`SELECT * FROM cmb_projects ORDER BY featured DESC, created_at DESC`
  } catch {
    return []
  }
}
